...
var TimeGrid = Grid.extend({
	...
	slatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			html +=
				'<tr  event-time="' + slotDate.format() + '" ' + (!minutes ? '' : 'class="fc-minor"') + '>' +
					(!isRTL ? axisHtml : '') +
...
$(document).ready(function() {
		...
		$calendar.fullCalendar({
            ...
            eventAfterAllRender: function() {
                ...
                $calendar.find(".fc-time-grid tr").bind("mousedown", function (e) {
                    if (e.which == 3) {
                        alert("You right-clicked on " + $(this).attr("event-time"));
                    }
                });
            }
		});
